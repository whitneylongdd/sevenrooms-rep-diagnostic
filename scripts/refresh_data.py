...
What each field means:
  ...
  cw26sqo  = # 2026 closes whose SQO was also in 2026 (numerator for '26 CVR)
  cwNN     = # Net New closes in 2026 (Opp Type Detail Category = "Net New")
  cwExp    = # Expansion closes in 2026
  mrr2026  = sum of MRR Final for 2026 closes
  sqo2026  = # SQOs created in 2026 (from Opps Created tab, Month of Opp SQO col)
  ace/jack/king = facecard counts for 2026 closes (Facecard Final col)
...
MONTHS COVERED: Jan–May 2026. Update MONTHS_2026 set below when Jun+ data arrives.
"""
...
# ── Update this set as new months close ──────────────────────
...
rep = defaultdict(lambda: {
    'cw2026':0,'cw26sqo':0,'cwNN':0,'cwExp':0,
...
for row in cw_rows[1:]:
    ...
    opp_type   = row[13].strip() if len(row) > 13 else ''  # col 13: Opp Type Detail Category
    sqo_mo_raw = row[34].strip() if len(row) > 34 else ''  # col 34: Opp SQO Month M/1/YYYY
    facecard   = row[42].strip() if len(row) > 42 else ''  # col 42: Facecard Final
    mrr_raw    = row[46].strip() if len(row) > 46 else ''  # col 46: MRR Final
...
for row in sqo_rows[1:]:
    ...
    if rep_name and sqo_mo in VALID_SQO_2026:
        rep_sqo[rep_name] += 1
...
# Col A = rep name, Col V (index 21) = Productivity Ramp
rep_ramp = {}
...
def patch_rep(m):
    ...
    # Ramp: use most recent ramp value from tracker
...
html = re.sub(r'\{id:"[^"]+",name:"[^"]+",.*?(?:mayCW:\d+)\}', patch_rep, html, flags=re.DOTALL)

with open(HTML_PATH, 'w') as f:
    f.write(html)

print("Done. Review changes with: git diff team.html")
print("Then commit: git add team.html && git commit -m 'Data refresh YYYY-MM-DD'")
